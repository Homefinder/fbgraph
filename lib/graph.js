...
function Facebook(options, req, res) {
  ...
  this.req = req;
  this.res = res;
  this.accessToken = req.session.facebookToken;
...
Graph.prototype.cleanUrl = function(url) {
  ...
  if (self.facebook.accessToken) {
    url += ~url.indexOf('?') ? '&' : '?';
    url += "access_token=" + self.facebook.accessToken;
  }
  return url;
};
...
Graph.prototype.get = function() {
  var self = this;
  request.get(this.requestOptions, function(err, res, body) {
    if (err) {
      self.callback({
...
Graph.prototype.post = function() {
  ...
  this.requestOptions.body = postData;
  request(this.requestOptions, function (err, res, body) {
    if (err) {
      self.callback({
          message: 'Error processing https request'
        , exception: err
